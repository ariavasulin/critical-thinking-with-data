...
## The HCE Audit (recurring assessment)

Every module ends with an HCE Audit — the same structured four-question response, applied to the dataset the module worked with. The audit is the course's single most important habit and happens three times (end of Modules 1, 2, and 3).
...
### Bridge 3: Module 2 HCE Audit — Cross-Module Synthesis

The Module 2 HCE Audit explicitly requires students to compare the Galton dataset (Module 1) to the sterilization dataset (Module 2) under the concepts of Classification and Performativity. This is the course's single required piece of written cross-module synthesis, and it lives inside the audit rather than as a separate essay.

---

## Inversion Moments

Each module contains at least one **inversion moment**: a single toggle in code whose output changes the student's interpretation of the entire analysis. These exist because ethical takeaways must live in the technical work, not alongside it.
